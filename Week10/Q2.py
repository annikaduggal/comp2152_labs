...
def get_failed_attempts(username):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM login_attempt WHERE username = ? AND success = 0", (username,))
...
def count_failures_per_user():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT username, COUNT(*) FROM login_attempt WHERE success = 0 GROUP BY username")
    rows = cursor.fetchall()
    conn.close()
    return rows
